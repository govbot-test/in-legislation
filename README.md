...
Formatted, versioned legislative data for Indiana. Part of the [govbot](https://github.com/chihacknight/govbot) civic data project.
...
This is the **formatted data repo** for Indiana — one of two repos per jurisdiction:

1. **Scraper repo** (`govbot-openstates-scrapers/in-legislation`) — runs [OpenStates](https://github.com/openstates/openstates-scrapers) scrapers, produces raw scraped JSON
2. **This repo** (`in-legislation`) — reads the scraper repo's output and:
...
Current settings for Indiana:

- **State code**: `in`
- **Toolkit branch**: `main`
- **Scraper repo**: `govbot-openstates-scrapers/in-legislation`
...
Pulls the latest scraped data from the scraper repo, sanitizes it, formats it into `country:us/state:in/sessions/...`, links events, and updates orphan tracking.
...
├── country:us/
│   └── state:in/            # state:usa for federal, state:il for Illinois, etc.
│       └── sessions/
│           └── {session_id}/
│               ├── bills/
│               │   └── {bill_id}/
...
Formatted metadata is saved to `country:us/state:in/sessions/`, organized by session and bill.
...
```json
{
  "HB999": {
    ...
    "vote_count": 2,
    "event_count": 0,
    "path": "country:us/state:in/sessions/103/bills/HB999"
  }
}
```
...
- Check the scraper repo (`govbot-openstates-scrapers/in-legislation`) actually got fresh data first — this repo only formats what that repo has already scraped
